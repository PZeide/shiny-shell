...
namespace Shiny::Services::BackgroundRemoval {
  ...
  void BackgroundRemovalUnit::setSource(QString source) {
    ...
    if (!newSource.isFile() || !newSource.isReadable()) {
      qWarning() << "Source is either missing, not a file or is not readable";
      return;
    }
  ...
  QString BackgroundRemovalUnit::result() const {
    ...
    return m_result->filePath();
  }

  void BackgroundRemovalUnit::start(bool ignoreCache) {
    if (!m_source || !m_result) {
      qWarning() << "Cannot process because source is missing";
      return;
    }
  ...
  void BackgroundRemovalUnit::processError(QProcess::ProcessError) {
    ...
    qWarning() << "The background removal process failed:" << m_runningProcess->errorString();
