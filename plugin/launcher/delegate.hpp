...
class LauncherDelegate : public QObject {
    ...
    // clang-format off
    Q_PROPERTY(QString input READ default WRITE default NOTIFY inputChanged BINDABLE bindableInput REQUIRED)
...
public:
    ...
    QBindable<QString> bindableInput();
...
private:
    ...
    // clang-format off
    Q_OBJECT_BINDABLE_PROPERTY(LauncherDelegate, QString, b_input, &LauncherDelegate::inputChanged)
    // clang-format on
    ...
    static void pluginsReplace(QQmlListProperty<LauncherPlugin>* property, qsizetype index, LauncherPlugin* value);
